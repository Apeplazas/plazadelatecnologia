	...
	<div class="swiper-wrapper">
		<div class="swiper-slide">
			...
			<ul id="listChange" class=" box" title="">
			  ...
			  <li>
			  	<a href="http://m.plazadelatecnologia.com/<?=strtolower($listaPro->rama)?>/oferta/<?=url_title(trim($listaPro->ofertaTitulo), '_')?>/<?=$listaPro->ofertaID?>">
			  	<p class="title"><?= word_limiter($listaPro->ofertaTitulo,6) ?> </p>
				  <span>
				  	<? if(fopen(base_url().'/ofertasLocatarios/'.str_replace('.', '_thumb.', $listaPro->ofertaImagen),'r')):?>
	         ...
	         <aside id="RamCatSub">
				<dl>
				  <dt>Marca</dt>
				    ...
				    <dd><a href="http://m.plazadelatecnologia.com/<?=$this->uri->segment(1)?>/<?= $marcaRow->marcaUrl?>/<?=$this->uri->segment(3)?>"><?=$marcaRow->marca?> (<?=$marcaRow->total?>)</a></dd>
				    <? endforeach;?>
				</dl>
				<? if($tematica):?>
				<dl>
					...
					<dd><a href="http://m.plazadelatecnologia.com/<?= $this->uri->segment(1);?>/<? if($this->uri->segment(2) != ''){ echo $this->uri->segment(2); }else{ echo '0';} ?>/<?= $rowTem->tematicaUrl;?>"><?= $rowTem->tematicaNombre;?></a></dd>
					<? endforeach; ?>
				</dl>
				<?endif;?>
				<? if($caracteristicas):foreach($caracteristicas as $caractRow):?>
				<dl>
				  ...
				  <dd><a href="http://m.plazadelatecnologia.com/<?=$this->uri->segment(1)?>/<? if ($this->uri->segment(2) != '0'):?><?= $this->uri->segment(2)?><? else:?>0<? endif?>/<? if ($this->uri->segment(3) != '0'):?><?= $this->uri->segment(3)?><? else:?>0<? endif?>/<?= $rowSub->catUrl;?>"><?= $rowSub->descripcion;?></a></dd>
				  <? else:?>
