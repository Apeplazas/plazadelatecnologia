...
class Ajax extends MX_Controller {
	...
	function ajax()
	{
		...
		$this->load->model('hobbits/hobbits_model');
	...
	{		
		...
		$tematica    	= $this->ofertas_model->obtenerIdTematica($tematicaNombre,$ramaID);
	...
	function cargarBancos()
	{
		...
		foreach($sc->result() as $row){
			...
			$comisionPT 	= (($precioLocal + $envio) * $comision)/100;
	...
	function cargarPagoBancos(){
		
		$tipoPago 	= $_POST['tipoPago'];
		$tipos		= $this->ajax_model->cargarAjaxTiposPago($tipoPago);
		echo json_encode($tipos);
		exit;
		
	}

	function cargaInfoPagoBanco(){
			
		$pagoId 		= $_POST['PagoId'];
		$detallePago	= $this->ajax_model->cargarAjaxDetallePago($pagoId);
		echo json_encode($detallePago[0]);	
		
	}
	
	function dinamicaHP()
	{
		$this->load->view('productoDinamicaHP-view');
	}
