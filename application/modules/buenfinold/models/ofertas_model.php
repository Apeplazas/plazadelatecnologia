...
class Ofertas_model extends CI_Model {
	...
	function cargarOfertasCat($plaza, $order){
		$data = array(); 
		...
		$q = $this->db->query("SELECT plaza, nombreCte, localcte, ofertaTitulo, ofertaDescripcion, precio, campLib1, campLib2, campLib3 
								FROM buenfin 
								WHERE plaza = '$plaza' ORDER BY '$order'");
	...
	function insertaUsuarios($usuario, $mail){
		$data = array(); 
		$q = $this->db->query("INSERT INTO buenfin_mailing (nombre, correo, sitio) VALUES ('$usuario', '$mail', 'pc')");
		
		return count($q);
	}
	
	function cargarDatoProd($producto){
		$data = array();
		
		$q = $this->db->query("SELECT * FROM buenfin WHERE id = '$producto' limit 1");
		if($q->num_rows() > 0) {
			foreach($q->result() as $row){
				$data[] = $row;
			}
	...
	function cargarOferta($plaza){
		$data = array(); 
		$q = $this->db->query("SELECT plaza, nombreCte, localcte, ofertaTitulo, ofertaDescripcion, precio, campLib1, campLib2, campLib3 
								FROM buenfin 
								WHERE plaza = '$plaza' ORDER BY RAND()");
	...
	function cargarOfertasPlaza($plaza){
		$data1 = '';
		$q = $this->db->query("SELECT id,
									  plaza, 
									  nombreCte, 
