...
<?= $this->session->flashdata('msg'); ?>
	...
	<div id="wrapDates">
	  <form id="wrapDate" method="get" action="<?=base_url()?>hobbits/buscarFecha">
		<fieldset><label>De:</label> <input class="fecha" name="de" placeholder="<?= $_GET['de']?>" type="text" id="d"></fieldset>
		<fieldset><label>a:</label> <input class="fecha" name="a" placeholder="<?= $_GET['a']?>" type="text" id="a"></fieldset>
	...
	<div id="panel">
		<div id="wrapPanel">
			...
			<? $solicitada 	= $this->hobbits_model->cargarPagadasTiposFechas('solicitado',$_GET['de'],$_GET['a']);?>
			<? $pendientes = 0;?>
			<? $totalVendido = 0;?>
			<? $pagadoLocal = 0;?>
			<? $comisionTransaccionBancos = 0;?>
			<? $totalGanancia = 0;?>		
			<? foreach($solicitada as $rowS):?>
			<? $comisionTransaccionBancos += $rowS->comisionBanco;?>
			<? $pagadoLocal += $rowS->pagoLocal;?>
			<? $totalVendido += $rowS->total;?>
			<? $totalGanancia += $rowS->gananciaPT;?>
			<? if($rowS->statusPagoLocal == 'no'):?><? $pendientes += $rowS->pagoLocal;?><? endif?>
			<? endforeach; ?>
			<? $totalSol = $totalVendido - $pagadoLocal;?>
			<? $banco = $totalSol - $pagadoLocal;?>
			
			
			
			<h1 class="mt20 mainRep">Información de traspasos</h1>
			<? $solPagadas 	= $this->hobbits_model->cargarPagadasSolicitadasFechas($_GET['de'],$_GET['a']);?>
			...
			<p><b>Pagos a locales</b> <em>$<?= number_format($totalVendido);?></em></p>
			<p><b>Dinero pagado a locales</b> <em>$<?= number_format($pagadoLocal);?></em></p>
			<p><b>Pagos Pendientes a locatarios</b> <em>$<?= number_format($pendientes);?></em></p>
			<p><b>Comisiones pagadas</b> <em>$<?= number_format($comisionTransaccionBancos);?></em></p>
			
			
			<p><b>Dinero en la Cuenta</b> <em>$<?= number_format($dinero);?></em></p>
			<p><b>Ganancias</b> <em>$<?= number_format($totalGanancia);?></em></p>
			
			<?$localesenlinea 	= $this->hobbits_model->solicitudLocalesenlineaFechas($_GET['de'],$_GET['a']);?>
			<!-- Solicitudes de locales -->
			<h1 class="mt20 mainRep">Locales en línea</h1>
