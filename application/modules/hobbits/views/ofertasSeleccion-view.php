...
<aside id="RamCatSub">
	<ul id="filtro">
		...
		<li><a href="<?=base_url()?>hobbits/segmento/<?= $this->uri->segment(3);?>"><img src="<?=base_url()?>assets/graphics/close.png" alt="Quitar Filtro" /><?= ucfirst( str_replace('_',' ', $this->uri->segment(4)));?></a></li>
		<? endif;?>
	...
	<dl>
	  <dt>Rama</dt>
	    ...
	    <dd><a href="<?=base_url()?>hobbits/segmento/<?=$this->uri->segment(3)?>/<?= $ramaRow->paginaUrl?>/"><?=$ramaRow->ramaNombre?> (<?=$ramaRow->total?>)</a></dd>
	    <? endforeach;?>
	</dl>
	<dl>
	  <dt>Marca</dt>
	    <? foreach($marcas as $marcaRow):?>
	    <dd><a href="<?=base_url()?>hobbits/segmento/<?=$this->uri->segment(3)?>/<?=$this->uri->segment(4)?>/<?= strtolower($marcaRow->marcaUrl);?>"><?=$marcaRow->marca?> (<?=$marcaRow->total?>)</a></dd>
	    <? endforeach;?>
	</dl>
	<dl>
		<dt>Categoria</dt>
		<? foreach($tematica as $rowTem):?>
		<dd><a href="<?=base_url()?>hobbits/segmento/Oferta/<?= $this->uri->segment(1);?>/<?= $this->uri->segment(2);?>/<?= $rowTem->tematicaUrl;?>"><?= $rowTem->tematicaNombre;?> (<?=$rowTem->total?>)</a></dd>
		<? endforeach; ?>
	</dl>
</aside>
