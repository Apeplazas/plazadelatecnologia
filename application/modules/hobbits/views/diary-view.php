	...
	<div id="panel">
	    ...
	    <tbody id="det">
			...
			<tr class="<?= $rowM->Status;?>">
		      ...
		      <td id="tipoMail<?= $rowM->mailID;?>"><?= $rowM->tipoMail;?></td>
		      <td id="ref<?= $rowM->mailID;?>" class="dateCam"><a href="<?=base_url();?>boletin/diary/<?= $rowM->mailID;?>" target="_blank">Ver mail</a></td>
		  	</tr>
	        <? endforeach; ?>
	    </tbody>
		</table>
		<? endif;?>
		<a class="agregaCam" href="<?=base_url()?>hobbits/agregaMailDiario"><img src="<?=base_url()?>assets/graphics/agregarCat.png" alt="Agregar campaña"> <span>Agregar Mail</span></a>
	</div>
</section>

<script type="text/javascript">
jQuery(function($){
<? foreach($mailDiario as $rowP):?>	
	// Actualiza status
	...
	// Actualiza tipo Mail
	$("#tipoMail<?= $rowP->mailID;?>").editInPlace({
		url: '<?=base_url()?>ajax/editTipoMail/<?= $rowP->mailID;?>',
		default_text: 'Cambia el tipo de Mail.',
		field_type: "select",
		bg_over: "#FFF1E2",
		select_options: "Mail Diario"
	});
<? endforeach; ?>
})
</script>

<? foreach($mailDiario as $rowB):?>
