...
<section id="artNot">
<h1><img src="<?=base_url()?>assets/graphics/<?= $this->uri->segment(2);?>-icon-articulos.png" alt="Noticias de <?= $this->uri->segment(2);?>" />Noticias interesantes de <?= str_replace('_',' ', $this->uri->segment(2));?></h1>
	<? foreach($noticias as $rowN):?>
	<article class="comment" id="<?=$rowN->articuloID;?>">
	<h2><?= $rowN->articuloTitulo;?></h2>
	<span>
	<a href="<?=base_url()?><?= $rowN->articuloTipo;?>/post/<?= $rowN->articuloUrl;?>"><img class="fleft" src="<?=base_url()?>articulosUpload/<?= $rowN->articuloImagenes;?>.jpg" alt="<?= $rowN->articuloTitulo;?>" /></a>
...
$(window).scroll(function(){
	...
	if($(window).scrollTop() + 1 >= $(document).height() - $(window).height()){
		...
		$('div#loadMore').show();
		$.ajax({
			url:"<?=base_url()?>noticias/cargarMasTematicas/<?= $this->uri->segment(2);?>/" +$(".comment:last").attr("id"),
