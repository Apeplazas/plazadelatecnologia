...
class Carritowork extends MX_Controller {
	
	function procesarPaypal(){
		
		$folio 	= $this->input->post('folioId');
		
		$compra = $this->carritowork_model->compra($folio);
		...
		$detalleCompra	= $this->carritowork_model->detalleCompra($folio);
	...
	function procesarComproPago(){
		...
		$detalleCompra 	= $this->carritowork_model->detalleCompra($referencia);
	...
	function procesarConekta(){
		...
		$folioCompra	= $this->carritowork_model->traeUltimoFolio($user['uid']);
		$detalleCompra	= $this->carritowork_model->detalleCompra($folioCompra);
		...
		if( isset($_POST['conektaChargeId']) && !empty($_POST['conektaChargeId']) ){
			...
			$respuesta 		= $conekta->makePayment($totalCompra * 100,$folioCompra, $_POST["conektaTokenId"]);	
	...
	function carritowork()
	{
		...
		$this->load->model('carritowork_model');	
	...
	function guardarRegistro()
	{
		...
		if (!$mail) {
			...
			$verifica	= $this->carritowork_model->verificaID($email);
	...
	function confirmaCarrito()
	{
		...
		$query = $this->carritowork_model->folios();
		if($query){
		...
		redirect('carritowork/entrega/'.$folio);
	}
	...
	function entrega()
	{
		...
		$op['compra']			= $this->carritowork_model->compra($this->uri->segment(3));
		$op['detalleCompra']	= $this->carritowork_model->detalleCompra($this->uri->segment(3));
		$op['estados']			= $this->carritowork_model->estados();
		$op['direcciones']		= $this->carritowork_model->direccionesUser($user['uid']);
	...
	function formas_pago()
	{
		...
		$ultimoSitio = $this->carritowork_model->ultimoSitio();
		
		$up = array('idSitio' => $ultimoSitio[0]->idSitio);
		$this->db->where('folio', $folio);
		$this->db->update('compras', $up);
		
		$qty = $this->carritowork_model->cantidadComprada($folio);
		
		foreach ($qty as $itemQ) {
			$item = $this->carritowork_model->itemExistencia($itemQ->ofertaID);
		...
		$op['folio'] 			= $folio;
		$op['detalleCompra']	= $this->carritowork_model->detalleCompra($folio);
		$op['compra']			= $this->carritowork_model->compra($folio);
	...
	function resppayworks() {
		...
		if(!empty($CONTROL_NUMBER)&&$CONTROL_NUMBER!=""){
			...
			$info 	= $this->carritowork_model->infoReservasPendientesPayworks($CONTROL_NUMBER);			
...
recomendamos NO se recomienda enviar la transacción a procesar a Payworks. Su reservación esta pendiente por falta de pago";
	...
	function procesarPago() {
		...
		$info2 	= $this->carritowork_model->infoComprador($referencia);		
		
		if($info2){					
			
			$cart	= $this->carritowork_model->detalleCompra($referencia);			
			$descripcionVenta = '';
...
¿Quieres saber más sobre lanzamientos y recomendaciones? Suscríbete a nuestro canal de <a href='https://www.youtube.com/user/PlazadelaTecnologia'>YouTube</a> y disfruta de nuestras reseñas en tecnología.";
	...
	function respuestaPayuFin() {	
		...
		$info 	= $this->carritowork_model->infoReservasPendientes();
...
//$this->db->update('compras', $opRes);
	...
	function pagoAutorizacion() {		
        ...
        $info2 	= $this->carritowork_model->infoComprador($s_trans_referencia);	
	...
	function enviar_aqui()
	{
		...
		$op['detalleCompra']	= $this->carritowork_model->detalleCompra($folio);
		$op['compra']			= $this->carritowork_model->compra($folio);
		$op['credito']			= $this->data_model->cargarMetodoCredito();
		$op['diferidos']		= $this->data_model->cargarMetodoDiferido();
		$op['debito']			= $this->data_model->cargarMetodoDebito();
	...
	function email_confirmation($referencia,$importe,$tipoPago){
		...
		$info 	= $this->carritowork_model->infoCliente($referencia);
		...
		$cart		= $this->carritowork_model->detalleCompra($folio);
