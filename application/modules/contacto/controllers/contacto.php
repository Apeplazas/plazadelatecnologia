...
class Contacto extends MX_Controller {
	...
	function index()
	{
		...
		//Vista//
		$this->layouts->index('contacto-view' ,$op);
	}
	...
	function formulario()
	{
		...
		//Vista//
		$this->layouts->index('formulario-view' ,$op);
	}
	...
	function devoluciones()
	{
		...
		//Vista//
		$this->layouts->index('devoluciones-view' ,$op);
	}
	...
	function quejasysugerencias()
	{
		...
		//Vista//
		$this->layouts->index('quejasysugerencias-view' ,$op);
	}
	...
	function guardarContacto()
	{
		...
		$this->email->to($email);
		$this->email->cc('contacto@plazadelatecnologia.com');
...
');
	...
	function guardarDevolucion()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			redirect('contacto/devoluciones');
		}
			...
			//Valida la informacion del contacto
			$cadena = $nombreContacto." ".$email." ".$telefonoContacto." ".$estadoContacto." ".$comentario." ".$motivo;
		...
		$this->db->insert('devoluciones', $op);
		
		$this->load->library('email');
		$this->email->set_newline("\r\n");
		$this->email->from('atencionaclientes@plazadelatecnologia.com', 'Contacto Plaza de la Tecnologia');
		$this->email->to('contacto@plazadelatecnologia.com');
		$this->email->subject('Solicitud de devolución - Plaza de la Tecnologia');		
		$this->email->message('
<html>
<head>
<title>Solicitud de Devolución - Plaza de la Tecnologia</title>
</head>
<body>
<p>El usuario '.$nombreContacto	.'!!!</p>
<p>Ha solicitado una devolución de mercancia/p>
</body>
</html>
');
	if($this->email->send())
...
');
	...
	function guardarContactoRenta()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			preg_match("/\b(href|declare|select|insert|somebody|OR|or|\=|sleep|http|www)\b/",$cadena,$registros);
			if(count($registros) == 0){
			...
			$this->email->from('contacto@plazadelatecnologia.com', 'Contacto Plaza de la Tecnologia');
			$this->email->to($email);
			$this->email->cc('ktetlalmatzi@apeplazas.com,ctoriz@apeplazas.com');
			$this->email->bcc('brodriguez@apeplazas.com,wbarrueta@apeplazas.com');
