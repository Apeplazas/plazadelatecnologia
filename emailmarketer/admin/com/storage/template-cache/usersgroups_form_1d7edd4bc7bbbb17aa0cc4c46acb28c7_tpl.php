						...
						<td>
							...
							<div class="hideOnSystemAdminSelect groupRecord_COMMON_NodeJoin">
								<select id="groupRecord_Permissions" name="record[permissions][]" multiple="multiple" class="ISSelectReplacement">
									<?php if(!function_exists("foreach_140273")){ function foreach_140273(&$tpl, $array){ if(is_array($array) || is_object($array)): foreach($array as $permissionGroupValue=>$permissionInGroups): $tpl->Assign('permissionGroupValue', $permissionGroupValue, false); $tpl->Assign('permissionInGroups', $permissionInGroups, false);  ?>
										<optgroup label="<?php echo $tpl->Get('permissionInGroups','text'); ?>">
											<?php if(!function_exists("foreach_20877")){ function foreach_20877(&$tpl, $array){ if(is_array($array) || is_object($array)): foreach($array as $permissionAreaValue=>$permissionAreaItem): $tpl->Assign('permissionAreaValue', $permissionAreaValue, false); $tpl->Assign('permissionAreaItem', $permissionAreaItem, false);  ?>
											 ...
											 <?php endforeach; endif;}} foreach_20877($tpl, $tpl->Get('permissionInGroups','children')); ?>
										</optgroup>
									 <?php endforeach; endif;}} foreach_140273($tpl, $tpl->Get('permissionList')); ?>
								</select>
							</div>
						</td>
					</tr>
								...
								<?php else: ?>
									<div <?php if($tpl->Get('record','segmentadmin')): ?>style="display:none;"<?php endif; ?>>
										<select name="record[access][segments][]" multiple="multiple" class="ISSelectReplacement ISSelectSearch">
											<?php $array = $tpl->Get('availableSegments'); if(is_array($array) || is_object($array)): foreach($array as $__key=>$each): $tpl->Assign('__key', $__key, false); $tpl->Assign('each', $each, false);  ?>
								...
								<?php else: ?>
									<div <?php if($tpl->Get('record','templateadmin')): ?>style="display:none;"<?php endif; ?>>
										<select name="record[access][templates][]" multiple="multiple" class="ISSelectReplacement ISSelectSearch">
											<?php $array = $tpl->Get('availableTemplates'); if(is_array($array) || is_object($array)): foreach($array as $templateid=>$templateName): $tpl->Assign('templateid', $templateid, false); $tpl->Assign('templateName', $templateName, false);  ?>
